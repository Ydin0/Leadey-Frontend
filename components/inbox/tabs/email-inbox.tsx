...
/** The email two-pane inbox — one tab of the unified Inbox shell. */
export function EmailInbox() {
  ...
  return (
    <div className="flex-1 flex rounded-[14px] border border-border-subtle overflow-hidden bg-surface min-h-0">
      {/* Thread list */}
      <div className="w-[340px] border-r border-border-subtle flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingList ? (
            ...
            threads.map((t) => (
              <ThreadRow key={t.id} thread={t} active={t.id === selectedId} onClick={() => setSelectedId(t.id)} />
            ))
          )}
        </div>
      </div>
