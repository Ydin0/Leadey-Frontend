...
function ActivityCard({ a }: { a: FunnelLeadActivity }) {
  return (
    <div className="rounded-xl border border-border-subtle bg-surface p-3.5">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-[10.5px] text-ink-faint shrink-0">{formatRelativeTime(a.timestamp)}</span>
      </div>
    </div>
  );
}
...
function TimelineRow({ item, last }: { item: FeedItem; last: boolean }) {
  ...
  return (
    <div className="flex gap-3.5 items-stretch">
      ...
      <div className={cn("flex-1 min-w-0", last ? "pb-0" : "pb-4")}>
        {item.kind === "call" ? <CallCard record={item.record} /> : <ActivityCard a={item.activity} />}
      </div>
    </div>
  );
}
...
export function LeadTimeline({ activities, callRecords, onAddNote }: LeadTimelineProps) {
